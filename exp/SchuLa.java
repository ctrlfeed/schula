...
public class SchuLa extends File implements Serializable{
	 ...
	 * @ansatz: SchuLa bildet den Single-point-of-Access, also den ausgewiesenen Anknuepfpunkt f�r 
	 ...
	 * Aktionen durchzuf�hren. Dies soll Folgendes bezwecken:
	 ...
	 * 	   des Programmes dar, die von au�en ueber den Aufruf der Klasse direkt angesteuert werden koennen.
	 *  2. gesch�tzter Bereich: in diesem Bereich sind anschlie�end Konstanten, Variablen, Funktionen 
	 *     und Methoden definiert, die nur f�r die Software ansteuerbar sein sollen.
	 ...
	 *     einer schwer zu erratenden Zeichenkette als notwendiger Parameter f�r diese kritischen Routinen sein,
...
public SchuLa(URI uri) {
	 ...
	 * Mitgegeben werden soll ein Wert der Klasse Schuh, in dem alle gesuchten Merkmale ausgepr�gt sind, die Anderen leer.
	 * Die Methode gibt Schuhe sortiert nach Anzahl der �bereinstimmenden Merkmale maximal in L�nge des Limits zur�ck.
	 * Ist Limit auf "0" gesetzt, werden alle Schuhe mitgegeben, die in mindestens einem Merkmal �bereinstimmen.
	...
	public Schuh[] getSchuhlist(Schuh Vergleichswert, int Limit){
		...
		for (Entry entry : schuhkatalog.entrySet()) {
			...
			if (((Schuh) entry.getValue()).getAnzahl() > 0){
				...
				//die Zahl der �bereinstimmenden Merkmale wird festgehalten, um eine Sortierung vornehmen zu k�nnen.
				int score = 0;
			...
			//trimmen ende, pr�fe n�chstes Element
		}
		//durchsuchen des Schuhkataloges und vorraetiger Elemente abgeschlossen.
		//das ergebnis wird aus der variablen Hashtable in ein Feld fixer Gr��e �berf�hrt.
	...
	public Fach[] platzfuer(int verstaumenge, boolean zusammenhaengend){
		...
		//wenn es nicht m�glich gewesen sein sollte die Verstaumenge in einem Regal unter zu bringen,
		//w�re zu erwarten, dass "resultset" nicht mit F�chern gef�llt w�re. F�r den Fall sollte eine nicht zusammenh�ngende Variante gew�hlt werden.
		if (resultset.size() < verstaumenge && zusammenhaengend){
			return platzfuer(verstaumenge-resultset.size(),false);
		} else {
			System.out.println("Unzureichend Lagerplatz f�r diese Menge.");
		}
...
//geschuetzte Objekte:
	...
	/* 
	 * @Itr: Um eine eindeutige, instanzweite Identifizierung zu erm�glichen, wird fuer alle wiederkehrende Elemente 
	 * je eine fortlaufende Nummer gefuehrt, die allerdings nicht direkt vergeben wird. Stattdessen ist sie ueber 
	 * eine jeweilige ID generierende Methode gesch�tzt automatisiert abrufbar (siehe untern)
	 */
	 ...
	 * Bei unterschiedlich gro�en F�chern sind mehrere separate "fiktiv getrennte" Regale zu erfassen.
	 */
	private static int RegalItr;
	
	/*
	 * @Lager: Das Lager wird mittels einer Hashtable von String(RegID) und dem Regal selbst abgebildet.
	 * Die Entscheidung zu dieser Umsetzung l�sst sich wie folgt begr�nden:
	 *  + Eine Hashtable ist beliebig um Elemente erweiterbar und bietet somit einen klaren Vortiel hinsichtlich
	 *    der Flexibilit�t und Erweiterbarkeit gegen�ber Feldern
	 *  + �ber eine Hashtable kann gezielt nach einem bestimmten Element innerhalb des Objektes gesucht werden.
	 *  + Eine Hashtable laesst einen Parallelen Zugriff zu. Unter dem Gedanken, dass perspektivisch mehrere Anfragen
	 *    parallel gestellt w�rden, lie�e dies einen performanten Vorteil zu.
	 *  + Eine Hashtable sichert von sich aus, dass sie l�ckenfrei bleibt und sichert somit Konsistenz.
	 *  Als Werte wurden die Klassen Regal und Schuh (unmittelbar) gew�hlt, da so das Objekt direkt referenziert wird und somit
	 *  die verwalteten Objekte nicht als separate Objekte und Klassen au�erhalb der Umgebung ansteuerbar sein muessen.
	 *  Als Key wurde jeweils die Regal-ID gewaehlt, um so die Transparenz �ber die Klassen zu halten.
	 *  Ein Regal, wie ein Schuh ist dabei grunds�tzlich durch die jeweilige ID eindeutig identifizierbar.
	 *    
	 */
	...
	/*
	 * @umlauf:  Defnition zweier Pseudo-regale, als unbestimmter Ort, an dem Schuhkartons verweilen k�nnen,
	 * allerdings dann nicht gezielt findbar sind. (trotzdem noch im Unternehmen, etwa in der Auslage oder Anprobe)
	 * Ver�u�erte Kartons werden dem Fach abgang zugeordnet, sodass die Kartons mit abgangsdatum nachvollziehbar 
...
// geschuetzte Methoden:
	 ...
	 * anschlie�end in die Verwaltung Lagerplatz unmittelbar aufgenommen.
	 * Das neu erfasste Regal ist dadurch zur Speicherung von Objekten verf�gbar. 
	 */
	void addRegal(){
		Regal r = new Regal();
	...
	void endRegal(Regal altRegal){
		if (altRegal.isEmpty()) {
			...
			System.out.println("In dem Regal befinden sich noch Elemente. Bitte entferne diese, um das Regal anschlie�end zu l�schen.");
		}
	}
	/*
	 * @addSchuh(): hinzuf�gen einer neuen Schuh-instanz als Eigenschaftskombination, der dann Kartons zugeordnet werden k�nnen.
	 */
	 ...
	 * Ohne spezifizierten R�ckgabewert findet, wird der Ort "umlauf" angenommen.
	 */
...
//		}
	 ...
	 * �bereinstimmung zur�ckgegeben. (Inventur) -- geschuetzt da Laufzeitkritisch
	 */
	Schuh[] Inventur(){
